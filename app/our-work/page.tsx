...
<section className={styles.section}><div className="shell"><div className={styles.sectionHead}><div><span className={styles.label}>Portfolio</span><h2>Choose a gallery.</h2><p>Tap into the type of event you’re planning and explore more examples of how the spaces can come together.</p></div><Link className="button button-primary" href="/#booking">Choose Your Date →</Link></div><div className={styles.gallery}>{gallery.map(([slug,label,image])=><Link key={slug} href={`/our-work/${slug}`} style={{display:'block'}}><figure><img src={image} alt={`${label} marquee event`} loading="lazy"/><figcaption>{label} <span aria-hidden="true">→</span></figcaption></figure></Link>)}</div></div></section>
